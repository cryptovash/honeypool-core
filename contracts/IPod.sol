...
/// @title The HoneyPool Pod specification
interface IPod is IERC20Upgradeable {
    ...
    /// @dev This function should deposit the float into the prize pool, and claim any HNYP tokens and distribute to users (possibly via adaptation of Token Faucet)
    ...
    /// @dev This function should disallow the withdrawal of tickets or HNYP to prevent users from being rugged.
    /// @param token The ERC20 token to withdraw.  Must not be prize pool tickets or HNYP tokens.
    ...
    /// @notice Allows a user to claim HNYP tokens for an address.  The user will be transferred their share of HNYP tokens.
